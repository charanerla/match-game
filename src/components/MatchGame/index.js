...
class MatchGame extends Component {
  ...
  updateActiveTabItem = tabId => {
    const activeTabDetails = tabsList.filter(tab => tab.tabId === tabId)
    this.setState({activeTabItem: activeTabDetails[0]})
  }
  ...
  displayGameResult = () => {
    ...
    return (
      <div className="game-result-container">
        ...
        <h1 className="your-score-number">{score}</h1>
        <button
          type="button"
  ...
  activeTabThumbnailItems = () => {
    const {activeTabItem} = this.state
    const listOfActiveThumbnailItems = imagesList.filter(
      eachObj => eachObj.category === activeTabItem.tabId,
    )
    return listOfActiveThumbnailItems
  }

  randomSelectionOfMatchImgObj = () => {
    const matchObj = Math.floor(Math.random() * imagesList.length)
    return imagesList[matchObj]
  }

  checkThumbnailIsMatched = id => {
    const {matchImgObj} = this.state
    if (matchImgObj.id === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        matchImgObj: this.randomSelectionOfMatchImgObj(),
      }))
    } else {
      this.setState({isGameOver: true})
    }
  ...
  startGame = () => {
    const {matchImgObj, activeTabItem} = this.state
    const listOfActiveThumbnailItems = this.activeTabThumbnailItems()
    return (
      <div className="matchImg-tabs-thumbnail-container">
        <img src={matchImgObj.imageUrl} alt="match" className="match-image" />
        <div className="tabs-container">
          <ul className="unordered-list-of-tab-items">
            {tabsList.map(tab => (
              <TabItem
                tabDetails={tab}
                key={tab.tabId}
                updateActiveTabItem={this.updateActiveTabItem}
                isActive={activeTabItem.tabId === tab.tabId}
              />
            ))}
          </ul>
        </div>
        <div className="thumbnails-container">
          <ul className="unordered-list-of-thumbnails">
            {listOfActiveThumbnailItems.map(eachObj => (
              <ThumbnailItem
                thumbnailDetails={eachObj}
                checkThumbnailIsMatched={this.checkThumbnailIsMatched}
                key={eachObj.id}
              />
            ))}
          </ul>
    ...
    return (
      <div className="match-game">
        <div className="header-container">
          ...
          <ul className="score-clock-img-time-container">
            <li>
              <p className="score">
                Score: <span className="span-elements">{score}</span>
              </p>
            </li>
            <li>
              <img
        ...
        <div className="body-container">
          {isGameOver ? this.displayGameResult() : this.startGame()}
        </div>
      </div>
    )
  }
